...
def parse_args():
  ...
  parser.add_argument('--summary_output_dir', type=str, 
    ...
    help='Relative or absolute directory path to output summary data for tensorboard.')
  # optimizations
...
def pool_layer(layer_name, layer_input):
  with tf.name_scope(layer_name):
    with tf.name_scope('{}_pooling'.format(args.pooling_type)):
...
def get_weights(vgg_layers, i):
  ...
  W = tf.constant(weights, name='weights')
  return W

def get_bias(vgg_layers, i):
  bias = vgg_layers[i][0][0][2][0][1]
  b = tf.constant(np.reshape(bias, (bias.size)), name='bias')
  return b
...
def content_layer_loss(p, x):
  with tf.name_scope('content_layer_loss'):
    _, h, w, d = p.get_shape()
    M = h.value * w.value
    N = d.value
    if args.content_loss_function   == 1:
      ...
      K = 1. / 2.
    with tf.name_scope('calculate_content_loss'):
...
def style_layer_loss(a, x):
  with tf.name_scope('style_layer_loss'):
    ...
    A = gram_matrix(a, M, N)
    G = gram_matrix(x, M, N)
    with tf.name_scope('calculate_style_loss'):
...
def mask_style_layer(a, x, mask_img):
  with tf.name_scope('mask_style_layer'):
    _, h, w, d = a.get_shape()
...
def sum_masked_style_losses(sess, net, style_imgs):
  with tf.name_scope('sum_masked_style_losses'):
    total_style_loss = 0.
...
def sum_style_losses(sess, net, style_imgs):
  with tf.name_scope('sum_style_losses'):
    total_style_loss = 0.
    weights = args.style_imgs_weights
...
def sum_content_losses(sess, net, content_img):
  with tf.name_scope('sum_content_losses'):
...
def get_longterm_weights(i, j):
  with tf.name_scope('longterm_weights'):
    c_sum = 0.
...
def sum_longterm_temporal_losses(sess, net, frame, input_img):
  with tf.name_scope('sum_longterm_temporal_losses'):
...
def sum_shortterm_temporal_losses(sess, net, frame, input_img):
  with tf.name_scope('sum_shortterm_temporal_losses'):
    x = sess.run(net['input'].assign(input_img))
    prev_frame = frame - 1
    w = get_prev_warped_frame(frame)
...
def sum_total_variation_losses(sess, net, input_img):
  with tf.name_scope('sum_total_variation_losses'):
  ...
  with tf.device(args.device), tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    with tf.name_scope('losses'):
      ...
      with tf.name_scope('total_loss'):
        ...
        L_total += theta * L_tv
        tf.summary.histogram('total_loss', L_total)

      # video temporal loss
      with tf.name_scope('video_temporal_loss'):
...
def get_summaries(sess):
  ...
  maybe_make_directory(args.summary_output_dir)
